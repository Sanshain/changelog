...
- `--filter=` - allows you to add custom filters (will useing as regexps) that will prevent matching commit names in changelog. By default, all short names (less then six symbols) are ignored, as well as names consisting of only one word, and also containing words  
